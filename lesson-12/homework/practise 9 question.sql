...
SELECT PARENTNAME from girls except select parentname from boys

select g.parentname from girls g left join  boys b on g.parentname=b.parentname where b.parentname is null
...
select custid,  sum(unitprice*qty*(1-discount)), min(weight) FROM  [tsql2012].[sales].[orders] o join [tsql2012].[sales].[orderdetails] od on o.orderid=od.orderid
where weight>50
group by cudtid
...
select isnull(c1.item, '') as[item cart 1], isnull(c2.item, '') as [item cart 2] from cart1 c1 full join order by c1.item desc
...
select name as customers from customers c left join  orders o on c.id=o.customeerid  where o.id is null
...
select s.student_id, s.student_name,sb.subject_name, couunt(e.student_id) attended_exams from students s cross join subjects sb left join examination e on s.student_id=e.student_id
